...
class Clustering {
    ...
    public function setConfig($centeroids = array(), $attributes = array(),$prefix){
    ...
    public function normalisasi(array $value,$demensi): array{
        $nData=count($value);
        $data=[];
        for ($i=0; $i <$nData ; $i++) { 
            for($j=0;$j<$demensi;$j++){
                $tmp=array_column($value,$j);
                $min=min($tmp);
                $max=max($tmp);
                $rumus=($value[$i][$j]-$min)/($max-$min);
                $data[$i][$j]=$rumus;
            }
        }
        return $data;
    ...
    public function countDistanceCenteroid() : array {
        try{
            ...
            for($cluster = 0;$cluster < count($this->attributes); $cluster++){
                ...
                for($centeroid = 0;$centeroid < count($this->centeroids); $centeroid++){
                    ...
                    for($attribute = 0;$attribute < count($this->attributes[$cluster]); $attribute++){
                        $distanceCenteroid = ($this->centeroids[$centeroid][$attribute] - $this->attributes[$cluster][$attribute]);
            ...
            //         echo Sprintf('Cluster %s Record %s : %s', $record+1, $cluster+1, $results[$cluster][$record])."<br>";
            //     }
            // }
    ...
    /**
     * array check centroidDiff 
     * @param array
     * @return bolean
     */
    public function centroidDiff(array $centroidOld,array $centroidNew,$variabel){
        for($i=0;$i<count($centroidOld);$i++){
            for($j=0;$j<$variabel;$j++){
                if($centroidOld[$i][$j]!==$centroidNew[$i][$j]){
                    return false;
                    break;
                }
            }
    ...
    public function determineCluster($clusterGroupings = array()) : array {
        ...
        //     echo Sprintf("Position %s with value %s", $resultsOfDetermine[$position]['position'], $resultsOfDetermine[$position]["value"])."<br>";
        // }
